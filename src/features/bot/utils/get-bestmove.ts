...
import { EngineResponse, MoveType } from "../../gameplay/types"
...
export function getBestMove(
    ...
    const promotion =
        bestMove.length === 5 ? (bestMove.slice(5) as PieceSymbol) : undefined
    ...
    if (lvl > 0 && lvl < 6) {
        ...
        if (isRandom) {
            ...
            const randomMove =
                possibleMoves[Math.floor(Math.random() * lastIndex)]
            return {
